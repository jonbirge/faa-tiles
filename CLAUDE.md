...
```bash
.venv/Scripts/python -m pytest                        # 209 tests, ~60s
...
tilesets: one button per tileset along the top and **nothing else** (the user
asked for no other controls), Cesium's rendering defaults except
...
`www/update_tilesets.py`, same format as `cesiumtiles-serve`), falling back to
